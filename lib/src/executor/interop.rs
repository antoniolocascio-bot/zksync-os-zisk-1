...
//!
//! - `sl_chain_id`  ← SystemContext (`0x800b`) slot 0 (static, written once at
//!   v31 activation). Here read at PRE-batch state (`tree_root_before`).
...
/// slot 0, authenticated against the PRE-batch tree root. `NonExisting` ⇒ 0.
pub(super) fn derive_sl_chain_id(proof: &StorageProof, tree_root_before: &B256) -> u64 {
    let flat_key = merkle::derive_flat_storage_key(&SYSTEM_CONTEXT_ADDRESS, &B256::ZERO);
    let value = verify_slot(proof, &flat_key, tree_root_before, "sl_chain_id").unwrap_or(B256::ZERO);
