//! FIX A — streaming deserialization of the storage-proof witness.
//!
...
//!
//! WIRE FORMAT IS UNCHANGED: the server still `bincode`-serialises `BatchInput`
//! verbatim. Only the guest's parsing differs. The field order below MUST match
//! the `#[derive(Deserialize)]` field order of `BatchInput`/`BlockInput` in
//! `types.rs`; the `stream_roundtrip_*` regression tests guard against drift.
...
impl<'de, 'a> Visitor<'de> for BlockVisitor<'a> {
    ...
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<BlockInput, A::Error> {
        ...
        // FIX A: stream storage_proofs — verify & drop each; collect nothing.
...
pub(super) fn stream_deserialize_and_build_db(
    ...
    // Same options as `bincode::deserialize`: fixint, little-endian (default),
    // allow trailing bytes (guest input is zero-padded to 8).
    ...
    let input: BatchInput = opts
        .deserialize_seed(BatchInputSeed { state: &mut state }, bytes)
    ...
    let block_hashes = proven_db::build_block_hashes(&input.blocks, meta);
...
mod tests {
    ...
    fn streaming_provendb_matches_collecting() {
        ...
        // Collecting path.
        let full: BatchInput = bincode::deserialize(&bytes).unwrap();
        ...
        assert_eq!(
            bincode::serialize(&stripped).unwrap(),
            bincode::serialize(&full_cleared).unwrap(),
