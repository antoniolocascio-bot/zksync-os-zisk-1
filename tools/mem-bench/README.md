...
- **FIX A (read-spam):** `execute_and_commit_from_bincode` (collecting:
...
- **FIX B (write-spam):** `BatchTreeUpdate::apply_reference` (pre-fix two-pass,
...
- FIX A peak **excludes** the serialized witness (it lives in the guest's
  read-only input region, not the heap).
- FIX B peak **includes** the deserialized `tree_update` witness (which is on
