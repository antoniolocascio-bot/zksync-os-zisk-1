...
//!
//! FIX A (read-spam): `bincode::deserialize::<BatchInput>` + `build_proven_db`
...
//! depth D.
//!
//! FIX B (write-spam): `BatchTreeUpdate::apply_reference` (the pre-fix two-pass
...
//! * FIX A peak = heap allocated DURING the run, EXCLUDING the serialized
//!   witness bytes (those live in the guest's separate read-only input region,
//!   not the heap).
//! * FIX B peak = the deserialized `tree_update` witness (which IS on the heap)
...
// --------------------------------------------------------------------------
// FIX B witness: W update writes spread across a depth-D tree.
// --------------------------------------------------------------------------
...
fn run_fix_a(ns: &[usize], ds: &[usize]) -> Vec<RowA> {
    ...
    for &n in ns {
        for &d in ds {
            ...
            eprintln!(
                "FIX A  N={:>7} D={:>2}  ser={:>7.1}  base={:>7.1}  fixed={:>6.2}  ({:.1}x)  t {:.0}/{:.0} ms",
...
fn run_fix_b(ws: &[u64], ds: &[u32]) -> Vec<RowB> {
    ...
    for &w in ws {
        for &d in ds {
            ...
            eprintln!(
                "FIX B  W={:>7} D={:>2}  witness={:>6.1}  base={:>7.1}  fixed={:>6.1}  ({:.1}x)  t {:.0}/{:.0} ms",
...
fn markdown(rows_a: &[RowA], rows_b: &[RowB], max_n: usize) -> String {
    ...
    // ---- FIX A ----
    s.push_str("## FIX A — streaming storage-proof deserialize (read-spam)\n\n");
    s.push_str(
    ...
    // ---- FIX B ----
    s.push_str("## FIX B — streaming O(W) tree update (write-spam)\n\n");
    s.push_str(
