#!/usr/bin/env bash
# Build, and optionally push, the three ZiSK stack images from the repo root.
#
...
#
# Options:
#   --targets "coordinator worker prover"  images to build (default: all three)
...
#
# The coordinator and worker images need nothing from this checkout beyond
# the Dockerfile: their binaries come from the pinned ZiSK tarball. The prover
# image copies out/zksync-os-zisk-guest, out/zksync-os-zisk-guest-aggregator
# and out/zksync-os-zisk-prover-service; the Dockerfile re-verifies the ELFs
...
TARGETS="coordinator worker prover"
REGISTRY=local
TAG=dev
PUSH=0
SKIP_GUEST_BUILD=0
...
for t in $TARGETS; do
    case "$t" in
        coordinator|worker) ;;
        prover) needs_prover=1 ;;
        *) die "unknown target: $t (expected coordinator, worker, prover)" ;;
    esac
done

if (( needs_prover )); then
    case "$PROVER_SOURCE" in
        ...
        bin)
            ...
            ;;
        "")
            die "the prover target needs --prover-from-docker, --prover-from-release TAG or --prover-bin PATH"
            ;;
    esac
...
for t in $TARGETS; do
    image="${REGISTRY}/zksync-os-zisk-${t}:${TAG}"
    echo "=== building ${image} (target ${t}, ${PLATFORM})"
    args=(buildx build --platform "$PLATFORM" -f "$DOCKERFILE" --target "$t" -t "$image")
